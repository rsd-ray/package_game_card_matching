...
class GameCardMatching extends StatefulWidget {
  const GameCardMatching({Key? key}) : super(key: key);
  static String _theme = 'dark';
  @override
  State<GameCardMatching> createState() => _GameCardMatchingState();
}

class _GameCardMatchingState extends State<GameCardMatching> {
  ...
  void _getTimes() async {
    ...
    if (_sp.getString('Theme') == 'light' && GameCardMatching._theme != 'light') {
      _changeTheme();
    }
    setState(() {});
  }
  ...
  void _changeTheme() async {
    setState(() {
      GameCardMatching._theme = (GameCardMatching._theme == 'dark') ? 'light' : 'dark';
    });
    SharedPreferences _sp = await SharedPreferences.getInstance();
    _sp.setString('Theme', GameCardMatching._theme);
  }

  @override
  Widget build(BuildContext context) {
    _responsiveCoefficient = sqrt(MediaQuery.of(context).size.width) * sqrt(MediaQuery.of(context).size.height);
    _complementColor = (GameCardMatching._theme == 'dark') ? const Color(0xFFBDBDBD) : const Color(0xFF212121);
    return Scaffold(
...
class _GameState extends State<Game> with TickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    _complementColor = (GameCardMatching._theme == 'dark') ? const Color(0xFFE0E0E0) : const Color(0xFF212121);
    _responsiveCoefficient = sqrt(MediaQuery.of(context).size.width) * sqrt(MediaQuery.of(context).size.height);
    _iconSize = _responsiveCoefficient / _dimension * 2;
    return SafeArea(
      child: Scaffold(
        backgroundColor: (GameCardMatching._theme == 'dark') ? Colors.black : const Color(0xFFE0E0E0),
        appBar: null,
        body: Stack(
          children: <Widget>[
            SlideTransition(
                          ...
                          Column(
                            ...
                            children: <Widget>[
                              ...
                              Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: <Widget>[
                                  Image.asset('assets/veteran1.gif', package: 'game_card_matching',),
                                  const SizedBox(width: 12.0),
                                  Image.asset('assets/veteran2.gif', width: 280.0, package: 'game_card_matching',),
                                ],
                              ),
                            ],
                          ),
                        if (widget._difficulty == 'CASUAL') Image.asset('assets/casual.gif', package: 'game_card_matching',),
                        if (widget._difficulty == 'EASY') Image.asset('assets/easy.gif', width: 280.0, package: 'game_card_matching',),
                        const SizedBox(height: 36.0),
                        Text(
                          _completeMessage,
                          textAlign: TextAlign.center,
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: <Widget>[
                            ...
                            const SizedBox(height: 36.0),
                            Image.asset('assets/tyler1-rage.gif', package: 'game_card_matching',),
                            const SizedBox(height: 36.0),
                            Row(
                              mainAxisSize: MainAxisSize.min,
                              children: <Widget>[
